...
class Message:
    ...
    def __init__(self, line_num, message):
        ...
        self.id = message.get('author', {}).get('id')
        ...
        Message.ids_and_names[self.id] = self.author
    ...
    def replace_id_with_name(self):
        ...
        for word in words:
            if word.startswith("<@") and word.endswith(">"):
                user_id = word[2:-1]
                name = Message.ids_and_names.get(user_id, f"@{user_id}")
    ...
    def __str__(self):
        ...
        if self.referenced_message_id:
            ...
            if reply and reply.get('author'):
                ...
                reply_info = f"Replying to @{reply_name} - "
        return message_line + reply_info + self.replace_id_with_name()

    #TODO add something to show there was an attachment and change from using .txt to .md for better looking stuff
...
def main():
    for file in INPUT_PATH.iterdir():
        if file.is_file():
            ...
            with open(OUTPUT_PATH / f"{file.name}Output.txt", 'w', encoding='utf-8') as f:
